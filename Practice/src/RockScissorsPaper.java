...
public class RockScissorsPaper {
	public static class PlayerCallable implements Callable<String> {
		String name;
		int call_sequence = 0;
		static String[] SelectionTable = { "Rock", "Scissors", "Paper" };
		...
		public String call() throws InterruptedException {
			...
			call_sequence++;
			System.out.format(
					"%s pauses %d microseconds on the %d-th invocation.\n",
	...
	/*
	 * public void run() { FutureTask player1 = new FutureTask(new
	 * ThisCallable()); }
	 */
	...
	public static String payoff(String first_hand, String second_hand) {
		if (first_hand.equals(second_hand)) {
			return String.format("'%s' from both hands is a tie.", first_hand);
		}
		if ((first_hand.equals("Rock") & second_hand.equals("Scissors"))
			...
			return String.format("One's '%s' beats Two's '%s'.", first_hand,
					second_hand);
		}
		return String.format("Two's '%s' beats One's '%s'.", second_hand,
				first_hand);
	}
	/*
	 * public class ThisCallable implements Callable { public Integer call()
